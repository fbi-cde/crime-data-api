...
SPOOL NPD.csv
select NPD.PROPERTY_ID PROP_DESC_ID, NPD.PROPERTY_VALUE, NPD.DATE_RECOVERED, NPD.NIBRS_PROP_DESC_ID from nibrs_property_desc NPD JOIN nibrs_property NP ON NP.PROPERTY_ID = NPD.PROPERTY_ID JOIN nibrs_incident NI ON NP.INCIDENT_ID = NI.INCIDENT_ID JOIN NIBRS_MONTH NM ON NI.NIBRS_MONTH_ID = NM.NIBRS_MONTH_ID WHERE DATA_YEAR = 2016;
SPOOL OFF
...
SPOOL NOF.csv
select NOF.OFFENDER_ID, NOF.INCIDENT_ID, NOF.OFFENDER_SEQ_NUM, NOF.AGE_ID, NOF.AGE_NUM, NOF.SEX_CODE, NOF.RACE_ID ETHNICITY_ID, NOF.FF_LINE_NUMBER, NOF.AGE_RANGE_LOW_NUM, NOF.AGE_RANGE_HIGH_NUM FROM nibrs_offender NOF JOIN nibrs_incident NI ON NOF.INCIDENT_ID = NI.INCIDENT_ID JOIN NIBRS_MONTH NM ON NI.NIBRS_MONTH_ID = NM.NIBRS_MONTH_ID WHERE DATA_YEAR = 2016;
SPOOL OFF
