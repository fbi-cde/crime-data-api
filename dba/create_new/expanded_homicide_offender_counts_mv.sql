CREATE MATERIALIZED VIEW shr_state_homicide_offender_count AS
select  state_id as state_id,
state_abbr as state_abbr,
data_year as data_year,
sum(count) as count
from public.shr_expanded_homicide_offender_count group by state_id, state_abbr, data_year;

CREATE MATERIALIZED VIEW shr_national_homicide_offender_count AS
select
data_year as data_year,
sum(count) as count
from public.shr_expanded_homicide_offender_count group by data_year;


CREATE MATERIALIZED VIEW shr_state_homicide_offender_sex AS
select  state_id as state_id,
state_abbr as state_abbr,
data_year as data_year,
...
CREATE MATERIALIZED VIEW shr_national_homicide_offender_sex AS
select
data_year as data_year,
coalesce(sum(case when sex_code = 'M' then count end), 0) as male_count,
coalesce(sum(case when sex_code = 'F' then count end), 0) as female_count,
...
CREATE MATERIALIZED VIEW shr_state_homicide_offender_race AS
select  state_id as state_id,
state_abbr as state_abbr,
data_year as data_year,
...
coalesce(sum(case when race_desc = 'Unknown' or race_desc = 'UNKNOWN' then count end), 0) as unknown,
...
CREATE MATERIALIZED VIEW shr_national_homicide_offender_race AS
...
CREATE MATERIALIZED VIEW shr_state_homicide_offender_age AS
select  state_id as state_id,
state_abbr as state_abbr,
data_year as data_year,
...
CREATE MATERIALIZED VIEW shr_national_homicide_offender_age AS
select
data_year as data_year,
coalesce(sum(case when age_range = '0-9' then count end), 0) as range_0_9,
coalesce(sum(case when age_range = '10-19' then count end), 0) as range_10_19,
...
CREATE MATERIALIZED VIEW shr_state_homicide_offender_ethnicity AS
select  state_id as state_id,
state_abbr as state_abbr,
data_year as data_year,
...
CREATE MATERIALIZED VIEW shr_national_homicide_offender_ethnicity AS
select data_year as data_year,
coalesce(sum(case when ethnicity_name = 'Hispanic or Latino' then count end), 0) as hispanic,
coalesce(sum(case when ethnicity_name = 'Multiple' then count end), 0) as multiple,
coalesce(sum(case when ethnicity_name = 'Not Hispanic or Latino' then count end), 0) as not_Hispanic,
coalesce(sum(case when ethnicity_name = 'Unknown' or ethnicity_name = 'UNKNOWN' then count end), 0) as unknown,
coalesce(sum(case when ethnicity_name = 'NR' or ethnicity_name = 'nr' then count end), 0) as not_reported
from public.shr_expanded_homicide_offender_count group by  data_year;

REFRESH MATERIALIZED VIEW shr_national_homicide_offender_ethnicity;
REFRESH MATERIALIZED VIEW shr_state_homicide_offender_ethnicity;
REFRESH MATERIALIZED VIEW shr_national_homicide_offender_age;
REFRESH MATERIALIZED VIEW shr_state_homicide_offender_age;
REFRESH MATERIALIZED VIEW shr_national_homicide_offender_race;
REFRESH MATERIALIZED VIEW shr_state_homicide_offender_race;
REFRESH MATERIALIZED VIEW shr_national_homicide_offender_sex;
REFRESH MATERIALIZED VIEW shr_state_homicide_offender_sex;
refresh MATERLIZED VIEW shr_national_homicide_offender_count;
refresh MATERLIZED VIEW shr_state_homicide_offender_count;
