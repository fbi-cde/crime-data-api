...
sum(  CASE  WHEN agency_data.PUBLISHABLE_FLAG = 'Y' THEN 1  ELSE 0 END) AS published_agency_count,
...
sum(  CASE  WHEN extract(year from  agency_data.nibrs_start_date) <= data_year THEN 1  ELSE 0 END) AS agency_count_nibrs_submitting,
...
COALESCE(  CASE  WHEN agency_data.PUBLISHABLE_FLAG = 'Y' THEN true ELSE false END) AS published,
COALESCE(  CASE WHEN agency_data.covered_flag = 'Y' THEN true  ELSE false END) AS covered,
COALESCE(  CASE  WHEN extract(year from  agency_data.nibrs_start_date) <= data_year  THEN true  ELSE false END) AS nibrs_submitting,
COALESCE(  CASE  WHEN agency_data.nibrs_start_date IS NOT NULL THEN nibrs_start_date  ELSE null END) AS nibrs_start_date,
COALESCE(  CASE  WHEN extract(year from  agency_data.nibrs_leoka_start_date) <= data_year  THEN true  ELSE false END) AS leoka_submitting,
