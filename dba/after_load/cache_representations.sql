CREATE OR REPLACE FUNCTION populate_nibrs_representation()
RETURNS VOID
AS
$BODY$
DECLARE
  rep_rec RECORD;
  rep_cur CURSOR FOR
    SELECT (representation->'incident_id')::text::bigint AS incident_id, representation
    FROM (
      ...
      FROM   (
        ...
        FROM     nibrs_incident
        WHERE    incident_id > ( SELECT MAX(incident_id) FROM nibrs_incident_representation)
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      VALUES (rep_rec.incident_id, rep_rec.representation);
      PERFORM pg_sleep(1);
      -- RAISE INFO 'Inserted record';
    EXCEPTION
      WHEN connection_exception THEN
        -- ROLLBACK;
        RAISE INFO 'Insert failed, check connection';
        PERFORM pg_sleep(10);
        RAISE INFO 'Retrying...';
    END;
  END LOOP;
