...
create materialized view hc_counts as select count(incident_id), offense_name, bias_name, year, county_id, state_id , agency_id
from ( SELECT DISTINCT(hc_incident.incident_id), bias_name, offense_name, hc_incident.agency_id, county_id, state_id, EXTRACT(YEAR FROM hc_incident.incident_date) as year from hc_incident 
...
create materialized view offense_hc_counts as select count(incident_id), offense_name, bias_name, year, county_id, state_id , agency_id
from ( SELECT DISTINCT(hc_incident.incident_id), bias_name, offense_name, hc_incident.agency_id, county_id, state_id, EXTRACT(YEAR FROM hc_incident.incident_date) as year from hc_incident 
    LEFT OUTER JOIN hc_offense ON hc_incident.incident_id = hc_offense.incident_id 
    LEFT OUTER JOIN hc_bias_motivation ON hc_offense.offense_id = hc_bias_motivation.offense_id 
    LEFT OUTER JOIN nibrs_offense_type ON nibrs_offense_type.offense_type_id = hc_offense.offense_type_id 
...
GROUP BY GROUPING SETS (
    ...
    (year, agency_id, offense_name, bias_name)
);