...
class TestAgenciesEndpoint:
    def test_agencies_endpoint_exists(self, testapp):
        res = testapp.get('/agencies/')
        assert res.status_code == 200

    def test_agencies_endpoint_returns_agencies(self, testapp):
        res = testapp.get('/agencies/')
    ...
    def test_agencies_endpoint_single_record_works(self, testapp):
        id_no = self._single_ori(testapp)
        res = testapp.get('/agencies/{}/'.format(id_no))
        assert res.status_code == 200

    def test_agencies_paginate(self, testapp):
    ...
    def test_agencies_page_size(self, testapp):
