...
class TestOffendersOffensesEndpoint:
    ...
    def test_offenders_offenses_endpoint_with_just_state_year(self, testapp, swagger_beta, variable):
    ...
    @pytest.mark.parametrize('variable', OffenseOffenderCountView.VARIABLES)
    def test_offenders_offenses_endpoint_with_state_postal_code(self, testapp, swagger_beta, variable):
    ...
    @pytest.mark.parametrize('variable', OffenseOffenderCountView.VARIABLES)
    def test_offenders_offenses_endpoint_with_state_year_offense(self, testapp, swagger_beta, variable):
    ...
    @pytest.mark.parametrize('variable', OffenseOffenderCountView.VARIABLES)
    def test_offenders_offenses_endpoint_with_agencies_year_offense(self, testapp, swagger_beta, variable):
        url = '/offenders/count/agencies/MI2336700/{}/offenses?offense_name=Aggravated+Assault&year=2014'.format(variable)
        res = testapp.get(url)
        #validate_api_call(swagger_beta, raw_request=res.request, raw_response=res)
        assert 'pagination' in res.json
        for r in res.json['results']:
    ...
    def test_offenders_offenses_endpoint_with_state_year_explorer_offense(self, testapp, swagger_beta, variable, explorer_offense):
