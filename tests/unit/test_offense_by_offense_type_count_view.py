...
class TestOffenseByOffenseTypeCountView:
    ...
    def test_count_for_a_state(self, app):
        v = OffenseByOffenseTypeCountView('weapon_name', year=1999, state_id=47, offense_name='Aggravated Assault', as_json=False)
    ...
    def test_endpoint(self, app, year, state_id, offense_name, variable):
        v = OffenseByOffenseTypeCountView(variable, year=year, state_id=state_id, offense_name=offense_name, as_json=False)
        results = v.query({}).fetchall()
        seen_values = set()
        # for row in results:
    ...
    def test_endpoint_for_explorer_offense(self, app, explorer_offense, variable):
        v = OffenseByOffenseTypeCountView(variable, year=1992, state_id=2, explorer_offense=explorer_offense, as_json=False)
        results = v.query({}).fetchall()
        if len(results) > 0:
            # seen_values = set()
            # for row in results:
    ...
    def test_explorer_offense_does_aggregation(self, app):
        v = OffenseByOffenseTypeCountView('location_name', year=2014, state_id=26, explorer_offense='larceny', as_json=False)
        results = v.query({}).fetchall()
